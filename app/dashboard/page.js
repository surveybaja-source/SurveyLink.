...
function InsurerDashboard({user}) {
  ...
  const sLabel = {searching:'Searching...',quoting:'Quotes received',accepted:'Surveyor assigned',completed:'Completed'}
    ...
    <div style={{background:'#0c1a27',minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 24px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,marginBottom:32}}>
          {[
            ['Active Requests',missions.filter(m=>m.status!=='completed').length,'','#f0a500'],
            ['Quotes Received',missions.filter(m=>m.status==='quoting').length,'awaiting review','#5a9eff'],
            ['Surveyors Assigned',missions.filter(m=>m.status==='accepted').length,'missions in progress','#2e7d32'],
          ...
          <div>
            ...
            {!loading&&missions.length===0&&(
              <div style={{background:'#132030',border:'1px solid #1e3a52',borderRadius:12,padding:36,textAlign:'center'}}>
                ...
                <div style={{color:'#4a6880',fontSize:12,marginTop:6,marginBottom:20}}>Click New Request to get started</div>
          ...
          {selected&&(
            <div>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                <h2 style={{color:'#fff',fontSize:22,fontWeight:800,margin:0}}>Quotes — {selected.reference}</h2>
                ...
                {quotes.map(q=>(
                  <div key={q.id} style={{background:'#132030',border:q.status==='accepted'?'1px solid #2e7d32':q.status==='declined'?'1px solid #700300':'1px solid #1e3a52',borderRadius:12,padding:16}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10}}>
                      ...
                      <div style={{background:'#0f1e2e',borderRadius:8,padding:'10px 14px'}}>
                        <div style={{color:'#4a6880',fontSize:9,marginBottom:3}}>ETA ON SITE</div>
  ...
  return (
    <div style={{background:'#0c1a27',minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 24px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,marginBottom:32}}>
          {[
            ['Available Missions',missions.length,'near you','#f0a500'],
            ['Quotes Submitted',myQuotes.length,'awaiting response','#5a9eff'],
            ['Active Surveys','0','in progress','#2e7d32'],
            ['Acceptance Rate','--','no data yet','#8fa8c0']
          ].map(([label,val,sub,color])=>(
            <div key={label} style={{background:'#132030',border:'1px solid #1e3a52',borderRadius:12,padding:'16px 20px'}}>
              <div style={{color:'#8fa8c0',fontSize:10,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:6}}>{label}</div>
              <div style={{color,fontSize:26,fontWeight:800}}>{val}</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:quoting?'1fr 1fr':'1fr',gap:24}}>
          <div>
            <h2 style={{color:'#fff',fontSize:22,fontWeight:800,marginBottom:16}}>Incoming Requests</h2>
            {loading&&<p style={{color:'#8fa8c0'}}>Loading...</p>}
            {!loading&&missions.length===0&&(
              <div style={{background:'#132030',border:'1px solid #1e3a52',borderRadius:12,padding:36,textAlign:'center'}}>
                <div style={{fontSize:32,marginBottom:12}}>📭</div>
                ...
                return (
                  <div key={m.id} style={{background:'#132030',border:alreadyQuoted?'1px solid #2e7d32':'1px solid #1e3a52',borderRadius:12,padding:20}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:14}}>
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                          <span style={{color:'#4a6880',fontSize:10}}>{m.reference}</span>
                          <span style={{background:m.urgency==='critical'?'rgba(221,46,30,0.12)':m.urgency==='urgent'?'rgba(240,165,0,0.12)':'rgba(26,108,240,0.12)',color:m.urgency==='critical'?'#dd2e1e':m.urgency==='urgent'?'#f0a500':'#5a9eff',padding:'2px 10px',borderRadius:4,fontSize:10,fontWeight:700,textTransform:'uppercase'}}>{m.urgency}</span>
                          {alreadyQuoted&&<span style={{background:'rgba(46,125,50,0.12)',border:'1px solid #2e7d32',color:'#81c784',padding:'2px 10px',borderRadius:4,fontSize:10,fontWeight:700}}>QUOTE SENT</span>}
                        </div>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:14}}>
                      {[['Location',m.location_text],['Client',m.client_name]].map(([k,v])=>(
            ...
            <div>
              ...
              <div style={{background:'#132030',border:'1px solid #1e3a52',borderRadius:12,padding:24}}>
                ...
                <div style={{marginBottom:16}}>
                  <div style={{color:'#8fa8c0',fontSize:10,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:6}}>Your Fee (EUR)</div>
                ...
                <div style={{marginBottom:12}}>
                  <input placeholder="ETA on site (e.g. 45 min, 2 hours)" value={eta} onChange={e=>setEta(e.target.value)}
