...
function InsurerProfile({profile, user}) {
  ...
  const SecT = ({text}) => (
    <div style={{color:'#1a1410',fontWeight:700,fontSize:13,letterSpacing:'0.08em',textTransform:'uppercase',margin:'20px 0 12px',paddingBottom:8,borderBottom:'1px solid #d8d4ce'}}>{text}</div>
  )

  return (
    <div style={{background:'#4a4640',minHeight:'100vh'}}>
      {toast&&<div style={{position:'fixed',top:70,left:'50%',transform:'translateX(-50%)',background:'#4a7a5a',color:'#fff',padding:'12px 24px',borderRadius:8,fontWeight:700,zIndex:999,fontSize:13}}>{toast}</div>}
      <nav style={{background:'#3a3630',borderBottom:'1px solid #5a5450',padding:'0 32px',height:58,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>router.push('/dashboard')} style={{background:'transparent',color:'#9a9490',border:'1px solid #5a5450',borderRadius:6,padding:'6px 16px',cursor:'pointer',fontSize:12}}>
            Back to Dashboard
          </button>
          <button onClick={()=>supabase.auth.signOut().then(()=>router.push('/auth'))} style={{background:'transparent',color:'#9a9490',border:'1px solid #5a5450',borderRadius:6,padding:'6px 16px',cursor:'pointer'}}>
            Sign Out
          </button>
        </div>
      </nav>
      <div style={{maxWidth:700,margin:'0 auto',padding:'32px 24px'}}>
        <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:32,padding:24,background:'#EDE9E4',border:'1px solid #d8d4ce',borderRadius:12,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}}>
          <div style={{width:64,height:64,borderRadius:'50%',background:'linear-gradient(135deg,#3a3630,#C4503A)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,fontWeight:800,color:'#fff'}}>
            🏢
          </div>
          <div>
            <div style={{color:'#1a1410',fontWeight:800,fontSize:22}}>{profile.company||'Your Company'}</div>
            <div style={{color:'#6a6460',fontSize:13,marginTop:2}}>{profile.email}</div>
            <div style={{marginTop:6}}>
              <span style={{background:profile.verified?'rgba(74,122,90,0.12)':'rgba(139,111,71,0.12)',border:`1px solid ${profile.verified?'#4a7a5a':'#8B6F47'}`,color:profile.verified?'#4a7a5a':'#8B6F47',padding:'2px 10px',borderRadius:4,fontSize:10,fontWeight:700}}>
        ...
        <div style={{background:'#EDE9E4',border:'1px solid #d8d4ce',borderRadius:12,padding:28,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div style={{marginBottom:14}}>
              ...
              <select value={f.country} onChange={e=>u('country')(e.target.value)}
                style={{width:'100%',background:'#f5f2ee',border:'1px solid #d8d4ce',borderRadius:6,padding:'10px 14px',color:f.country?'#1a1410':'#9a9490',boxSizing:'border-box',fontSize:13}}>
          ...
          <div style={{marginTop:8}}>
            <div style={{background:'rgba(196,80,58,0.08)',border:'1px solid #C4503A',borderRadius:8,padding:'11px 14px',marginBottom:16,display:'flex',gap:10}}>
              <span>ℹ️</span>
              <span style={{color:'#6a6460',fontSize:11,lineHeight:1.5}}>To update your email address or legal documents, please contact support@inspelink.com</span>
            </div>
            <button onClick={save} disabled={loading}
              style={{width:'100%',background:loading?'rgba(74,122,90,0.45)':'#4a7a5a',color:'#fff',border:'none',borderRadius:7,padding:'14px',cursor:'pointer',fontWeight:700,fontSize:14}}>
...
function ExpertProfile({profile, user}) {
  ...
  const save = async () => {
    ...
    await supabase.from('profiles').update({
      ...f, languages, certifications, specialties, commodities, equipment,
  ...
  return (
    <div style={{background:'#4a4640',minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:700,margin:'0 auto',padding:'32px 24px'}}>
        <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:32,padding:24,background:'#EDE9E4',border:'1px solid #d8d4ce',borderRadius:12,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}}>
          <div style={{width:64,height:64,borderRadius:'50%',background:'linear-gradient(135deg,#3a3630,#C4503A)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,color:'#fff',fontWeight:800}}>
            {profile.first_name?.[0]}{profile.last_name?.[0]}
          </div>
          <div style={{flex:1}}>
            <div style={{color:'#1a1410',fontWeight:800,fontSize:22}}>{profile.first_name} {profile.last_name}</div>
            <div style={{color:'#6a6460',fontSize:13,marginTop:2}}>{profile.company} — {profile.city}, {profile.country}</div>
            <div style={{color:'#9a9490',fontSize:12,marginTop:2}}>{profile.email}</div>
            <div style={{marginTop:6}}>
              <span style={{background:profile.verified?'rgba(74,122,90,0.12)':'rgba(139,111,71,0.12)',border:`1px solid ${profile.verified?'#4a7a5a':'#8B6F47'}`,color:profile.verified?'#4a7a5a':'#8B6F47',padding:'2px 10px',borderRadius:4,fontSize:10,fontWeight:700}}>
          ...
          <div style={{textAlign:'right'}}>
            <div style={{color:'#9a9490',fontSize:9,marginBottom:2}}>COVERAGE</div>
            <div style={{color:'#C4503A',fontWeight:800,fontSize:20}}>{coverage.length}</div>
            <div style={{color:'#9a9490',fontSize:9}}>countries</div>
          </div>
        </div>
        ...
        <div style={{display:'flex',gap:4,marginBottom:20}}>
          {TABS.map(tab=>(
            <button key={tab.k} onClick={()=>setActiveTab(tab.k)}
              style={{background:activeTab===tab.k?'#C4503A':'transparent',color:activeTab===tab.k?'#fff':'#9a9490',border:`1px solid ${activeTab===tab.k?'#C4503A':'#5a5450'}`,borderRadius:6,padding:'7px 16px',fontSize:12,cursor:'pointer',fontWeight:700}}>
              {tab.l}
            </button>
          ))}
        </div>
        ...
        <div style={{background:'#EDE9E4',border:'1px solid #d8d4ce',borderRadius:12,padding:28,boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}}>
          ...
          {activeTab==='coverage'&&(
            <div>
              ...
              <p style={{color:'#6a6460',fontSize:12,marginBottom:16,lineHeight:1.6}}>
                Select all countries where you are available to conduct surveys.
              </p>
          ...
          {activeTab==='banking'&&(
            <div>
              ...
              <div style={{background:'rgba(196,80,58,0.08)',border:'1px solid #C4503A',borderRadius:8,padding:'11px 14px',marginBottom:14,display:'flex',gap:10}}>
                <span>ℹ️</span>
                <span style={{color:'#6a6460',fontSize:11,lineHeight:1.5}}>To update your email address or legal documents, please contact support@inspelink.com</span>
              </div>
            </div>
          ...
          <div style={{marginTop:20,paddingTop:20,borderTop:'1px solid #d8d4ce'}}>
            <button onClick={save} disabled={loading}
              style={{width:'100%',background:loading?'rgba(74,122,90,0.45)':'#4a7a5a',color:'#fff',border:'none',borderRadius:7,padding:'14px',cursor:'pointer',fontWeight:700,fontSize:14}}>
              {loading?'Saving...':'Save Changes'}
            </button>
